  ...
  export class HistoryComponent implements OnInit {
    ...
    getDiffs(index: number, array: string[]): Promise<string[]> {
      return new Promise((resolve, reject) => {
        ...
        .getSingleCommitWithSha(this.loginName, this.repoName, this.commits[index]["sha"])
        .forEach(res => {
          let tmpIndex = res["files"].findIndex(file => file["filename"] == this.fileName);
          if (tmpIndex !== -1) {
    ...
    setup(): void {
      ...
      this.githubApiService
        ...
        .then(res => {
          this.getDiffs(this.commitIndex, new Array<string>())
            .then(res => {
